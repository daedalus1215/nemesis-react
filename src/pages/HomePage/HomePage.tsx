...
export const HomePage: React.FC = () => {
  ...
  const { data: balance, isLoading: isBalanceLoading, error: balanceError } = useUserBalance();
  ...
  if (error) return <div style={{ color: 'red' }}>{getErrorMessage(error)}</div>;
  ...
  if (isBalanceLoading) {
    ...
    balanceContent = <div className={styles.balance} style={{ color: 'red' }}>{getErrorMessage(balanceError)}</div>;
  } else if (balance) {
    balanceContent = <div className={styles.balance}>Balance: ${balance.amount}</div>;
  }
